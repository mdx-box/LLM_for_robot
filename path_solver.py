...
# ====================================
# #m目标函数
# 这个函数 objective 主要用于优化控制点（pose）的参数，使得一个机器人或物体的运动路径符合一定的要求。通过最小化成本函数，
# 优化控制点的参数来达到目标状态。这个函数的核心是优化算法中的 目标函数，通常用于机器人的运动规划或路径优化。
def objective(opt_vars, #优化变量，这是一个数组，包含了所有需要优化的参数
                og_bounds, #优化边界
                start_pose, #开始位姿
                end_pose, #结束位姿
                keypoints_centered, 
                ...
                sdf_func, #sdf函数
                collision_points_centered, #碰撞点
                opt_interpolate_pos_step_size, #插值位姿步长 
                opt_interpolate_rot_step_size, #插值旋转步长 控制插值精度
                ik_solver, #ik求解器
                initial_joint_pos, #初始关节位置
                reset_joint_pos, #重置关节位置
                return_debug_dict=False):

    debug_dict = {}
    debug_dict['num_control_points'] = len(opt_vars) // 6 #每个控制点由 6 个变量表示（3 个位置和 3 个旋转角度）

    # unnormalize variables and do conversion
    unnormalized_opt_vars = unnormalize_vars(opt_vars, og_bounds) #使用 unnormalize_vars 函数将优化变量 opt_vars 从标准化空间转换回原始空间。og_bounds 提供了原始的变量范围
    #将 start_pose 和 end_pose 作为路径的起始和结束点，unnormalized_opt_vars 是中间控制点的优化结果。所有控制点（包括起始和结束位姿）按行拼接成一个二维数组 control_points_euler，每行代表一个控制点的欧拉角表示
    ...
    # get dense samples 基于控制点的齐次矩阵和四元数生成路径的密集采样（样本）。opt_interpolate_pos_step_size 和 opt_interpolate_rot_step_size 控制插值的精度，返回的 poses_quat 是四元数表示的位姿样本，num_poses 是路径上的样本数量
    ...
    #将四元数表示的路径 poses_quat 转换为齐次变换矩阵 poses_homo
    poses_homo = T.convert_pose_quat2mat(poses_quat)
    #将路径样本的数量 num_poses 存入调试字典
    ...
    if collision_points_centered is not None:
        #如果存在碰撞点，调用 calculate_collision_cost 计算路径是否与障碍物发生碰撞，计算碰撞成本并加到总成本中。0.20 是距离阈值，表示碰撞的容忍度。
    ...
    # reachability cost
    #计算逆向运动学成本：对每个控制点，使用 ik_solver 求解对应的关节位置，并计算其误差。计算成功率和运动学的惩罚。,如果逆向运动学成功，还计算关节位置和重置关节位置之间的差异，并加上惩罚。
    ik_cost = 0
    reset_reg_cost = 0
    ...
    # # path constraint violation cost 路径约束惩罚
